...
| 属性 | 值 |
|------|-----|
| **父项目** | holdings-briefing v14.56（子模块指针，Plan: `woolly-herding-reef.md`） |

### 背景

宏观指标「上证指数」`000001.SH` 在 `_efinance_secid()` 里被**代码前缀启发式**误判：剥后缀后 code=`000001` 以 `0` 开头 → 判为深圳 `0.000001` = 平安银行（股价 ~11.4），而非上证指数 `1.000001`（约 3300+）。实测 `data/2026-08-21/macro.json` 里 `000001.SH` price=11.41 被污染，直接带偏 DSA 自选股周报第③段宏观周环比。

### 改动文件

| 文件 | 改动 |
|------|------|
| `scripts/fetch.py` | `_efinance_secid()`：显式 `.SH`/`.SZ` 后缀优先于代码前缀启发式（`000001.SH → 1.000001`）；无后缀时沿用原启发式 |

### 验证

- `000001.SH→1.000001`、`002008.SZ→0.002008`、`600519.SH→1.600519`、`513010`(无后缀)→`1.513010`、`300750`→`0.300750` ✅
- 回归安全：全项目唯一受影响 A 股符号即 `000001.SH`；A 股持仓 `513010/588080/588710` 无后缀、以 5 开头 → 仍 `1.xxxxx` 沪市，行为不变 ✅
- 观察验证：08-22 `fetch-daily` 后 `macro.json` 的 `000001.SH` price 应 ≈3300

### 回滚方法

```bash
git revert <commit>   # 恢复代码前缀启发式（000001.SH 回到平安银行，宏观段继续污染）
```
